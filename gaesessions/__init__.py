...
def verifysid(session):
	if not session.has_key('sid'):
		session['sid'] = random.choice('AB') + random.choice('CD') + '-' + ''.join(random.choice(string.ascii_lowercase + string.digits) for x in range(12))# only show the message once

_tls = threading.local()
def get_current_session():
    """Returns the session associated with the current request."""
    verifysid(_tls.current_session)
