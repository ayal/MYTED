...
DEFAULT_LIFETIME = datetime.timedelta(days=365)
...
def verifyPairCode(session):
	if not session.has_key('paircode'):
		logging.debug('generating pair code')
		session['paircode'] = random.choice('AB') + random.choice('CD') + '-' + ''.join(random.choice(string.ascii_lowercase + string.digits) for x in range(8))# only show the message once
	logging.debug('paircode is %s', session['paircode'])

_tls = threading.local()
def get_current_session():
    """Returns the session associated with the current request."""
    verifyPairCode(_tls.current_session)
...
class DjangoSessionMiddleware(object):
    ...
    def __init__(self):
        ...
        self.wrapped_wsgi_middleware = SessionMiddleware(fake_app, cookie_key='7c35cc96-56be-4aa1-afc8-9a0f403f23a1')
